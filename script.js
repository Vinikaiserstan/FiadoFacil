...
function navegar(id) {
    ...
    if(id === 'venda') {
        ...
        s.innerHTML = db.clientes.map((c, i) => `<option value="${i}">${c.nome}</option>`).join('');
    }
...
document.getElementById('form-cliente').onsubmit = (e) => {
    e.preventDefault();
    db.clientes.push({ nome: document.getElementById('nome-cliente').value, tel: document.getElementById('tel-cliente').value.replace(/\D/g,'') });
...
function renderizarHistorico() {
    ...
    db.vendas.forEach((v, i) => {
        ...
        const msg = encodeURIComponent(`Olá ${v.nome}, lembrete de pagamento: R$ ${v.valor.toFixed(2)}. Status: ${motivo || 'Pendente'}`);

        container.innerHTML += `
            <div class="item-venda ${deveNotificar ? 'notificar-agora' : ''}">
                <div>
                    <strong>${v.nome}</strong> ${v.pago ? '✅' : ''}<br>
                    <small>Vencimento: ${v.vencimento}</small><br>
                    ${deveNotificar ? `<span class="badge">${motivo}</span>` : ''}
                </div>
                <div style="text-align:right">
                    <strong>R$ ${v.valor.toFixed(2)}</strong><br>
                    <div style="margin-top:10px">
                        ${!v.pago ? `<button onclick="quitar(${i})" style="padding:5px">✓</button>` : ''}
...
function quitar(i) { db.vendas[i].pago = true; salvar(); renderizarHistorico(); }
