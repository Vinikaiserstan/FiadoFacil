...
// Controle visual do campo parcelas
...
document.getElementById('form-venda').addEventListener('submit', (e) => {
    ...
    db.vendas.push({
        ...
        vencimento: document.getElementById('vencimento-venda').value,
        parcelas: isParcelado ? document.getElementById('qtd-vezes').value : "À vista",
        pago: false
    });
...
function renderizarHistorico() {
    ...
    if (!container) return;
    container.innerHTML = db.vendas.length ? '' : '<p>Nenhum registro encontrado.</p>';
    
    [...db.vendas].reverse().forEach((v, i) => {
        const indexReal = db.vendas.length - 1 - i;
        const statusClass = v.pago ? 'status-pago' : '';
        const msg = encodeURIComponent(`Olá ${v.nome}, lembrete da sua conta: R$ ${v.valor.toFixed(2)}. Vencimento: ${v.vencimento}`);
        
        container.innerHTML += `
            <div class="item-venda ${statusClass}">
                <div class="info-venda">
                    <strong>${v.nome} ${v.pago ? '✅' : ''}</strong><br>
                    <small>${v.data} | ${v.desc}</small><br>
                    <small>Vencimento: ${v.vencimento} | Parcelas: ${v.parcelas}</small>
                </div>
